...
void cruzar(string matriz[]){
    int inicial = 0, final= 9; //se declaran los valores iniciales de inicio y final, que están en posiciones opuestas
    int pcruce;
    string padre1,padre2,hijo1,hijo2; //se crean los padres e hijos

    while(inicial!=5){ //solo se puede hacer el cambio cinco veces, de 0-4
        ...
        cout<<"Hijos "<<inicial+1<<endl;
        cout<<hijo1<<endl;
        cout<<hijo2<<endl<<endl;
        
        cout<<"Hijos mutados "<<inicial+1<<endl;
        ...
        else if(contarceros(hijo2)>contarceros(padre1)){ //usamos contarceros para saber si el h1 tiene más 0's que el p1
            matriz[inicial] = hijo2;
            if(contarceros(hijo1)>contarceros(padre2)){ //si ocurre, ahora compararemos el h2 con el p2
                matriz[final] = hijo1;
            }
        }
        else if(contarceros(hijo2)>contarceros(padre2)){ //usamos contarceros para saber si el h1 tiene más 0's que el p2
            matriz[final] = hijo2;
            if(contarceros(hijo1)>contarceros(padre1)){ ///si ocurre, ahora compararemos el h2 con el p1
                matriz[final] = hijo1;
            }
        }
        inicial++;
